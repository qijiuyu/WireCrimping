...
import com.bdkj.wirecrimping.activity.setting.SupervisionActivity;
...
public class AddSupervisionDialog extends Dialog {
    ...
    public AddSupervisionDialog(Context context) {
        super(context);
        this.context = context;
    }

    public AddSupervisionDialog(Context context, int themeStyle, NameBean editBean) {
        super(context, themeStyle);
        this.context = context;
        this.editBean = editBean;
    }

    protected AddSupervisionDialog(Context context, boolean cancelable, OnCancelListener cancelListener) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.dialog_add_supervision);
        ...
        //获取保存的所有数据
        final String SUPERVISION= SPUtil.getInstance(context).getString(Constant.SUPERVISION);
        if (!TextUtils.isEmpty(SUPERVISION)) {
            dataBeanList.addAll(JsonUtil.stringToList(SUPERVISION, NameBean.class));
        }
        ...
        findViewById(R.id.tv_submit).setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                ...
                SPUtil.getInstance(context).addString(Constant.SUPERVISION, JsonUtil.objectToString(dataBeanList));
                /**
                 * 更新列表页
                 */
                if(context instanceof SupervisionActivity){
                    ((SupervisionActivity)context).showListData();
                }
                dismiss();
            }
        });
